...
function CursorLayer() {
  ...
  useEffect(() => {
    ...
    const onOver = (e: PointerEvent) => {
      ...
      // Independent of the interactive-state lookup below: which surface
      // (light or dark) is nearest the pointer. A single combined selector
      // means closest() stops at whichever marker is nearer in the DOM —
      // required because Hero's outer pinned wrapper carries `dark-panel`
      // for its reveal-frame effect while the actual visible content sits in
      // a nested `bg-paper` section; without `.bg-paper` in the list, a plain
