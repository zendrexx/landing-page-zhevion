...
export function ProductProjectCard({ product, number, category, screenIndexes, tone, reverse = false }: { product: StudioProduct; number: string; category: string; screenIndexes: [number, number]; tone: "grocery" | "forge"; reverse?: boolean }) {
  ...
  return <article className={`grid gap-6 border-t border-ink/20 pt-6 lg:grid-cols-2 lg:items-center ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}><div><p className="text-sm font-semibold text-ink-faint">{number} / {category}</p><h3 className="mt-4 text-4xl font-extrabold tracking-[-.055em]">{product.name}</h3><p className="mt-4 max-w-lg text-base leading-relaxed text-ink-soft">{product.pitch}</p>{product.learnMoreHref ? <a href={product.learnMoreHref} className="text-link mt-5 inline-block text-sm font-bold">View project →</a> : null}</div><div className={`dark-panel flex gap-4 overflow-hidden rounded-card p-5 ${tone === "grocery" ? "bg-forest-500" : ""}`}>{screens.map((screen) => <DeviceFrame key={screen.src} src={screen.src} alt={screen.alt} imgWidth={product.screenSize.w} imgHeight={product.screenSize.h} width={210} chrome={product.screenChrome} bandIncluded={product.screenBandIncluded} sizes="210px" className="min-w-0" />)}</div></article>;
}

export function PortfolioBigCard({ project, number, reverse = false }: { project: PortfolioProject; number: string; reverse?: boolean }) {
  return <article className={`grid gap-6 border-t border-ink/20 pt-6 lg:grid-cols-2 lg:items-center ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}><div><p className="text-sm font-semibold text-ink-faint">{number} / Portfolio work</p><h3 className="mt-4 text-3xl font-extrabold tracking-[-.05em]">{project.name}</h3><p className="mt-4 max-w-lg text-base leading-relaxed text-ink-soft">{project.summary}</p><p className="mt-4 text-sm font-semibold text-ink-soft">{project.tags.join(" · ")}</p><a href={project.href} target="_blank" rel="noreferrer" className="text-link mt-5 inline-block text-sm font-bold">View project →</a></div><div className="relative aspect-[16/10] overflow-hidden rounded-card border border-ink/10 bg-paper-deep"><Image src={project.image} alt="" fill sizes="(min-width: 1024px) 50vw, 100vw" className="object-cover" /></div></article>;
}
